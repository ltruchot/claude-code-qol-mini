...
Colours and the gauge follow fixed token thresholds, not a share of the window:
...
GREEN = "\033[32m"
ORANGE = "\033[38;5;208m"   # 256-colour orange, distinct from the warning yellow
RED = "\033[91m"
...
def from_transcript(path):
    """Last main-thread assistant turn's context size, or 0.
...
def main():
    ...
    colour = RED if used >= ALERT_AT else ORANGE if used >= WARN_AT else GREEN
    filled = min(BAR_WIDTH, max(0, round(used / ALERT_AT * BAR_WIDTH)))
    gauge = f"{colour}{'▓' * filled}{RESET}{DIM}{'░' * (BAR_WIDTH - filled)}{RESET}"

    count = f"{colour}{BOLD}{'! ' if used >= ALERT_AT else ''}{human(used)}{RESET}"
